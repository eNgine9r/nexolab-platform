...
const equipmentId = requiredEnvironment("NEXOLAB_REMOTE_EQUIPMENT_ID");
const apiBaseUrl = requiredUrl("NEXOLAB_REMOTE_API_URL");
const storageOrigin = requiredOrigin("NEXOLAB_REMOTE_STORAGE_ORIGIN");
const websocketUrl = requiredUrl("NEXOLAB_REMOTE_WEBSOCKET_URL", ["wss:"]);
const expectedOperatorLogin = requiredEnvironment("NEXOLAB_EXPECTED_OPERATOR_LOGIN");
...
async function openEquipment(page: Page, version: number) {
  await page.goto(equipmentRoute, { waitUntil: "networkidle" });
  await expect(page.getByTestId("operator-session")).toContainText(expectedOperatorLogin);
...
test("runs the real Tailscale TLS operator acceptance flow", async ({ browser }) => {
  ...
  try {
    await test.step("verify trusted Tailscale operator session and WSS", async () => {
      await openEquipment(pageA, 1);

      const sessionResponse = await operatorA.request.get(`${apiBaseUrl}/api/v1/operator/session`, {
        headers: { "X-Actor-Id": "spoofed-remote-actor" },
      });
      expect(sessionResponse.status()).toBe(200);
      const session = (await sessionResponse.json()) as OperatorSessionPayload;
      expect(session).toMatchObject({
        actor_id: expectedOperatorLogin,
        provider: "tailscale",
        authenticated: true,
      });
      ...
      await pageA.evaluate(async (url) => {
        await new Promise<void>((resolve, reject) => {
          ...
          socket.addEventListener("open", () => {
            ...
            socket.close(1000, "remote acceptance");
            resolve();
          });
      ...
      writeJson("operator-session.json", {
    ...
    await test.step("upload through HTTPS and verify external MinIO signed URL", async () => {
      await pageA.getByLabel("Вибрати production-фото обладнання").setInputFiles({
        name: "remote-central-acceptance.png",
        mimeType: "image/png",
      ...
      expect(parsedSignedUrl.protocol).toBe("https:");
      expect(parsedSignedUrl.origin).toBe(storageOrigin);
    ...
    await test.step("persist 48 placements and publish with trusted operator identity", async () => {
      ...
      expect(history.items[0]).toMatchObject({
        ...
        published_by: expectedOperatorLogin,
      });
      expect(history.items[0]?.placements).toHaveLength(48);
    });

    await test.step("recover a parallel stale-writer conflict without losing local state", async () => {
      await openEquipment(pageB, 4);
      await enterEditMode(pageA);
      ...
      await pageB.screenshot({
        path: path.join(evidenceDirectory, "remote-conflict-local-state-preserved.png"),
        fullPage: true,
      });
      ...
      await pageB.screenshot({
        path: path.join(evidenceDirectory, "remote-conflict-server-version-reloaded.png"),
        fullPage: true,
      });
    });

    await test.step("record final remote PostgreSQL-backed API state", async () => {
      ...
      writeJson("remote-browser-acceptance-summary.json", {
        equipmentId,
        operatorLogin: expectedOperatorLogin,
        dashboardOrigin: new URL(requiredEnvironment("NEXOLAB_REMOTE_DASHBOARD_URL")).origin,
        ...
        publishedBy: history.items[0]?.published_by,
...
function writeJson(fileName: string, value: unknown) {
  writeFileSync(path.join(evidenceDirectory, fileName), `${JSON.stringify(value, null, 2)}\n`);
}

function requiredEnvironment(name: string): string {
  const value = process.env[name]?.trim();
  if (!value) throw new Error(`${name} is required for remote acceptance.`);
  return value;
}

function requiredOrigin(name: string): string {
  return requiredUrl(name).replace(/\/$/, "");
}

function requiredUrl(name: string, allowedProtocols: string[] = ["https:"]): string {
