...
class WorkflowMatrixTests(unittest.TestCase):
    def run(self, *, run_id: int, workflow_id: int, name: str, status: str, conclusion=None, created="2026-08-22T10:00:00Z", attempt=1):
        return WorkflowRun(
            id=run_id,
    ...
    def test_latest_by_workflow_excludes_current_core_run(self) -> None:
        runs = [
            self.run(run_id=10, workflow_id=1, name="CI", status="completed", conclusion="failure"),
            self.run(
                run_id=11,
                workflow_id=1,
                name="CI",
            ...
            self.run(run_id=20, workflow_id=2, name="Offline Bundle", status="completed", conclusion="success"),
        ]
    ...
    def test_latest_by_workflow_uses_newest_run_for_each_workflow(self) -> None:
        runs = [
            self.run(run_id=20, workflow_id=2, name="Offline Bundle", status="completed", conclusion="failure"),
            self.run(
                run_id=21,
                workflow_id=2,
                name="Offline Bundle",
    ...
    def test_pending_workflow_is_not_green(self) -> None:
        pending, failed = MODULE.evaluate_runs(
            [self.run(run_id=20, workflow_id=2, name="Offline Bundle", status="in_progress")]
        )
    ...
    def test_failure_cancel_and_skip_are_merge_blocking(self) -> None:
        runs = [
            self.run(run_id=20, workflow_id=2, name="Offline Bundle", status="completed", conclusion="failure"),
            self.run(run_id=30, workflow_id=3, name="Browser", status="completed", conclusion="cancelled"),
            self.run(run_id=40, workflow_id=4, name="Security", status="completed", conclusion="skipped"),
        ]
    ...
    def test_only_successful_completed_workflows_are_green(self) -> None:
        runs = [
            self.run(run_id=20, workflow_id=2, name="Offline Bundle", status="completed", conclusion="success"),
            self.run(run_id=30, workflow_id=3, name="Browser", status="completed", conclusion="success"),
        ]
        pending, failed = MODULE.evaluate_runs(runs)
        self.assertEqual(pending, [])
        self.assertEqual(failed, [])
