...
class TelegramGatewayRefreshPolicyTests(unittest.TestCase):
    ...
    def test_refresh_requires_exact_source_image_root_and_approval(self) -> None:
        ...
        fetch = self.text.index('"${GIT_AS_INVOKER[@]}" fetch --quiet origin main')
        remote = self.text.index("REMOTE_MAIN=")
        self.assertLess(fetch, remote)
        self.assertIn("SUDO_UID", self.text)
        self.assertIn("SUDO_GID", self.text)
        self.assertIn('getent passwd "$SUDO_UID"', self.text)
        self.assertIn('stat -c \'%u\' "$REPO_ROOT"', self.text)
        self.assertIn('runuser -u "$INVOKING_GIT_USER" -- git -C "$REPO_ROOT"', self.text)
        self.assertIn("unable to refresh origin/main authority as invoking Git user", self.text)
