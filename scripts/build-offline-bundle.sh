...
mkdir -p "$STAGING"/{deploy/compose,deploy/offline,deploy/systemd,images,evidence,scripts,docs}
...
cp infrastructure/systemd/nexolab-update-check.service "$STAGING/deploy/systemd/"
cp infrastructure/systemd/nexolab-update-check.timer "$STAGING/deploy/systemd/"
...
cp scripts/nexolab-update-orchestrator.py "$STAGING/scripts/"
cp scripts/deploy-version-manager-service.sh "$STAGING/scripts/"
cp scripts/deploy-capacity-guard.sh "$STAGING/scripts/"
