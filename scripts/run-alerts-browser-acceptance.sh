...
MQTT_VOLUME_NAME="${PROJECT_NAME}-mqtt-data"
POSTGRES_VOLUME_NAME="${PROJECT_NAME}-postgres-data"
OBJECT_STORAGE_VOLUME_NAME="${PROJECT_NAME}-object-storage-data"
...
cleanup() {
  ...
  compose logs --no-color telemetry-service postgres mqtt >"${TMPDIR:-/tmp}/${PROJECT_NAME}-services.log" 2>&1 || true
  compose down --volumes --remove-orphans >/dev/null 2>&1 || true
  if [[ $status -ne 0 ]]; then
    echo "Alerts browser acceptance failed. Frontend log:" >&2
    tail -n 160 "$FRONTEND_LOG" >&2 || true
    echo "Service log:" >&2
    tail -n 220 "${TMPDIR:-/tmp}/${PROJECT_NAME}-services.log" >&2 || true
  fi
  exit "$status"
}
trap cleanup EXIT
...
export ACCEPTANCE_MQTT_VOLUME_NAME="$MQTT_VOLUME_NAME"
export ACCEPTANCE_POSTGRES_VOLUME_NAME="$POSTGRES_VOLUME_NAME"
export ACCEPTANCE_OBJECT_STORAGE_VOLUME_NAME="$OBJECT_STORAGE_VOLUME_NAME"
export ACCEPTANCE_NETWORK_NAME="$NETWORK_NAME"
...
export NEXT_PUBLIC_NEXOLAB_SUPABASE_URL=""
export NEXT_PUBLIC_NEXOLAB_SUPABASE_ANON_KEY=""
export NEXT_PUBLIC_NEXOLAB_ACCEPTANCE_TOKEN_KEY="nexolab.acceptance.access_token"
export NEXT_PUBLIC_NEXOLAB_ACCEPTANCE_ORGANIZATION_KEY="nexolab.acceptance.organization_id"
