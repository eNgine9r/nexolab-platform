...
The equipment route starts one reconciliation cycle per equipment identity and stable repository instance. Mutable equipment state is not an effect dependency, preventing successful snapshot responses from recursively restarting hydration and access checks. Detail reconciliation is keyed only by the equipment identity, chamber identity and explicit mutation epoch; the sensor array it refreshes is not a dependency of that same effect. Empty and error fallbacks use functional state updates so React dependency analysis remains strict without recreating the reconciliation loop.
