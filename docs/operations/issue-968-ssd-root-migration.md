...
### First SSD boot failure and recovery — 2026-09-08

The first real SSD boot reached the copied ext4 root on `/dev/sda2`, but initramfs then failed before switching root because the freshly formatted target did not contain the empty `/dev`, `/proc`, `/sys`, `/run`, `/tmp`, `/mnt`, and `/media` directories. The root-copy rsync correctly excluded pseudo/runtime-tree contents, but the migration helper did not recreate their mountpoints. Console evidence showed `/dev/sda2` mounting successfully followed by `mounting /dev on /root/dev failed: No such file or directory`, equivalent failures for `/run`, `/sys`, and `/proc`, then `/root/dev/console` missing and a kernel panic.

Rollback to the untouched microSD succeeded. Read-back from the SSD confirmed the exact missing directories; the SSD filesystems, PARTUUIDs, USB 3 transport, kernel storage support and copied data remained intact. The already-prepared SSD was repaired in place by recreating only the required empty mountpoints with source-matching modes (`/proc` and `/sys` `0555`, `/tmp` `1777`, the remaining root-owned directories `0755`). `e2fsck -fn /dev/sda2` and `fsck.fat -n /dev/sda1` then returned clean status. Repository hardening is tracked by Issue #970 and must ensure every root copy recreates and validates these mountpoints before cutover. No SSD reformat, product-data deletion, Docker-volume deletion, Modbus write or hardware write is required for this recovery.
