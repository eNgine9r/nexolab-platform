...
const AGENT_CADENCE_PATH = "/api/v1/acquisition-cadence";
          ...
          detail: {
            ...
            message:
              response.status === 401
                ...
                : "Недостатньо прав для керування інтервалом фізичного опитування.",
          },
        },
      ...
      denied: NextResponse.json(
        {
          detail: {
            code: "access_denied",
            message: "Недостатньо прав для керування інтервалом фізичного опитування.",
          },
        },
        { status: 403 },
      ),
...
async function relayAgent(
  ...
  try {
    ...
    response = await fetch(new URL(AGENT_CADENCE_PATH, agentBaseUrl()), {
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  return relayAgent({ method: "GET", timeoutMs: 10_000 });
}
    ...
    return NextResponse.json(
      {
        detail: {
          ...
          message: "Некоректний розмір зміни інтервалу збору даних.",
        },
      },
      { status: 422 },
    );
