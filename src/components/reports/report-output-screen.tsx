...
import { ReportOutputWorkspace } from "./report-output-workspace";

export function ReportOutputScreen({ reportId }: { reportId: string }) {
  ...
  if (
    ...
    return <OutputSecurityGate state={security.state} error={security.error} onRetry={security.retry} />;
  }
      ...
      <div className="min-h-screen lg:pl-[264px]">
        ...
        <main className="relative overflow-hidden p-3 sm:p-4 xl:p-5 2xl:p-6">
          ...
          <div className="relative mx-auto max-w-[1500px]">
            <ReportOutputWorkspace
              key={`${security.membership?.organizationId ?? "report-output"}:${reportId}`}
              reportId={reportId}
            />
          </div>
        </main>
