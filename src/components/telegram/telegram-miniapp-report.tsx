...
export function TelegramMiniAppReport() {
  ...
  return (
    <main className="min-h-dvh bg-[#06142a] px-3 pt-3 pb-10 text-slate-100 sm:px-5">
      <Script
  ...
  return (
    <header className="mb-3 flex items-center justify-between rounded-2xl border border-cyan-300/10 bg-[#0a1f3d]/90 px-4 py-3 shadow-2xl shadow-black/20">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-[10px] font-semibold tracking-[0.2em] text-cyan-200/70 uppercase">NEXOLAB</p>
    ...
    <div className="space-y-3">
      <section className="panel p-4">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
          <InfoCell label="Дата звіту" value={formatDate(localDate)} icon={<Clock3 className="size-4" />} />
          <InfoCell
            label="Сформовано"
        ...
        <div className="mt-2 rounded-xl border border-slate-400/10 bg-black/10 px-3 py-2.5">
          <p className="text-[10px] tracking-[0.12em] text-slate-500 uppercase">Вікно даних · {timezone}</p>
          <p className="mt-1 text-xs font-medium text-slate-200">
            {formatDateTime(windowStart, timezone)} → {formatDateTime(windowEnd, timezone)}
          </p>
        </div>
      </section>

      {quality.status === "incomplete" || qualityReasons.length ? (
        <QualityWarning reasons={qualityReasons} />
      ) : null}

      <nav className="flex scrollbar-thin gap-2 overflow-x-auto pb-1 text-[11px]" aria-label="Розділи звіту">
        {[
          ["#m-packets", "М-пакети"],
          ["#summary", "Показники"],
          ["#circuit", "Контур"],
          ...
          <a
            key={href}
            href={href}
            className="shrink-0 rounded-full border border-slate-400/10 bg-white/[0.025] px-3 py-1.5 text-slate-300"
          >
            {label}
          </a>
        ))}
      ...
      <section id="m-packets" className="panel scroll-mt-3 p-4">
        <SectionTitle
          icon={<ThermometerSnowflake className="size-4" />}
          title="М-пакети"
          subtitle={countRatio(packets.valid_channels, packets.configured_channels, "валідних")}
        />
        ...
        <div className="mt-3 space-y-2">
          {channels.length ? (
            channels.map((channel, index) => (
              <ChannelRow
                key={`${text(channel.channel_id) ?? "channel"}-${index}`}
                channel={channel}
                timezone={timezone}
              />
            ))
          ) : (
            <UnavailableRow text="Список M-пакетів недоступний у цьому snapshot." />
          )}
        </div>
      </section>

      <section id="summary" className="panel scroll-mt-3 p-4">
        <SectionTitle
          icon={<BarChart3 className="size-4" />}
          title="Підсумок за вікно"
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <MetricCard
            ...
            label="Компресор"
            value={percent(compressor.duty_percent, compressor.status)}
            subvalue={coverage(compressor.coverage_percent)}
          />
          <MetricCard
            icon={<Bolt className="size-4" />}
            label="Енергія"
            value={energyValue(energy)}
            subvalue={energyEvidence(energy)}
          />
          <MetricCard
            icon={<RefreshCw className="size-4" />}
            label="Відтайка"
            value={duration(defrost)}
            subvalue="тільки тривалість"
          />
          <MetricCard
            icon={<AlertTriangle className="size-4" />}
            label="Тривоги"
            value={countValue(alerts.active_count, "активних")}
            subvalue={countValue(alerts.recent_count, "у вікні")}
          />
        </div>
      </section>

      <section id="circuit" className="panel scroll-mt-3 p-4">
        <SectionTitle
          icon={<Snowflake className="size-4" />}
          title="Холодильний контур"
          subtitle="непідтверджені RFX-метрики не обчислюються"
        />
        <div className="mt-3 grid grid-cols-2 gap-2">
          <MetricCard
            label="Кипіння"
            value={derivedMetric(circuit.evaporation_saturation_temperature, "°C")}
          />
          <MetricCard label="Перегрів" value={derivedMetric(circuit.superheat, "K")} />
          <MetricCard
            label="Конденсація"
            value={derivedMetric(circuit.condensation_saturation_temperature, "°C")}
          />
          <MetricCard label="Переохолодження" value={derivedMetric(circuit.subcooling, "K")} />
        </div>
      </section>

      <section id="alerts" className="panel scroll-mt-3 p-4">
        <SectionTitle
          icon={<AlertTriangle className="size-4" />}
          title="Тривоги"
          subtitle={`${countValue(alerts.active_count, "активних")} · ${countValue(alerts.recent_count, "у вікні")}`}
        />
        <div className="mt-3 space-y-2">
          {alertItems.length ? (
            alertItems
              .slice(0, 12)
              .map((item, index) => (
                <AlertRow key={`${text(item.id) ?? "alert"}-${index}`} item={item} timezone={timezone} />
              ))
          ) : (
            <UnavailableRow text="Активних або недавніх тривог у вікні звіту немає." />
          )}
        </div>
      </section>

      <footer className="px-2 pt-1 text-center text-[10px] leading-4 text-slate-500">
        NEXOLAB Mini App показує persisted snapshot. Керування обладнанням, підтвердження тривог і
        адміністративні дії відсутні.
      </footer>
    </div>
  );
}
...
function MetricCard({
  ...
  return (
    <div className="rounded-xl border border-slate-400/10 bg-white/[0.025] p-3">
      <div className="flex items-center gap-1.5 text-[10px] tracking-[0.1em] text-slate-500 uppercase">
        {icon ? <span className="text-cyan-200">{icon}</span> : null}
        {label}
      </div>
      <p className="mt-1.5 text-base font-semibold text-white">{value}</p>
      {subvalue ? <p className="mt-1 text-[10px] text-slate-500">{subvalue}</p> : null}
    </div>
  );
}
...
function ChannelRow({ channel, timezone }: { channel: JsonRecord; timezone: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-slate-400/10 bg-black/10 px-3 py-2.5">
      <div className="min-w-0">
        <p className="truncate text-xs font-medium text-slate-200">
          {text(channel.label) ?? text(channel.channel_id) ?? "M-пакет"}
        </p>
        <p className="mt-0.5 truncate text-[10px] text-slate-500">
          {available
            ? formatDateTime(text(channel.captured_at), timezone)
            : unavailableReason(text(channel.reason))}
        </p>
      </div>
      <div className="text-right">
        <p className={`text-sm font-semibold ${available ? "text-white" : "text-slate-500"}`}>
          {available ? temperature(channel.value_c) : "Недоступно"}
        </p>
        <p
          className={`mt-0.5 text-[9px] tracking-[0.08em] uppercase ${available ? "text-emerald-300/80" : "text-slate-600"}`}
        >
...
function AlertRow({ item, timezone }: { item: JsonRecord; timezone: string }) {
  ...
  const severityClass =
    severity === "critical" ? "text-red-200" : severity === "warning" ? "text-amber-200" : "text-slate-300";
  return (
    <div className="rounded-xl border border-slate-400/10 bg-black/10 px-3 py-2.5">
      <div className="flex items-center justify-between gap-2">
        <p className={`text-[10px] font-semibold tracking-[0.1em] uppercase ${severityClass}`}>
          {severityLabel(severity)}
        </p>
        <span className="text-[10px] text-slate-500">
          {formatDateTime(text(item.triggered_at), timezone)}
        </span>
      </div>
      <p className="mt-1 truncate text-xs text-slate-300">
        {text(item.channel_id) ?? text(item.equipment_id) ?? "Обладнання"} · {text(item.metric) ?? "Тривога"}
      </p>
    </div>
  );
}
...
function QualityWarning({ reasons }: { reasons: string[] }) {
  return (
    <section className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.06] px-4 py-3">
      <div className="flex gap-2.5">
        <AlertTriangle className="mt-0.5 size-4 shrink-0 text-amber-200" />
        <div>
          <p className="text-xs font-semibold text-amber-100">Неповні дані</p>
          <p className="mt-1 text-[11px] leading-4 text-amber-100/65">
            {reasons.length
              ? reasons.map(qualityReason).join(" · ")
              : "Частина evidence у вікні звіту недоступна."}
          </p>
        </div>
      </div>
    </section>
...
function UnavailableRow({ text: value }: { text: string }) {
  return (
    <div className="rounded-xl border border-dashed border-slate-400/10 px-3 py-3 text-center text-[11px] text-slate-500">
      {value}
    </div>
  );
}
...
function countValue(value: unknown, suffix: string): string {
  const n = integer(value);
  return n === null ? `— ${suffix}` : `${n} ${suffix}`;
}
function countRatio(valid: unknown, total: unknown, suffix: string): string {
  const validCount = integer(valid);
  const totalCount = integer(total);
  return validCount === null || totalCount === null
    ? "coverage недоступне"
    : `${validCount} / ${totalCount} ${suffix}`;
}
function temperature(value: unknown): string {
  const n = finite(value);
  return n === null ? "Недоступно" : `${n.toFixed(1)} °C`;
}
function coverage(value: unknown): string {
  const n = finite(value);
  return n === null ? "coverage недоступний" : `coverage ${n.toFixed(1)} %`;
}
function percent(value: unknown, status: unknown): string {
  const n = finite(value);
  return status !== "available" || n === null ? "Недоступно" : `${n.toFixed(1)} %`;
}
function energyValue(section: JsonRecord): string {
  const n = finite(section.interval_kwh);
  return section.status !== "available" || n === null ? "Недоступно" : `${n.toFixed(2)} kWh`;
}
function energyEvidence(section: JsonRecord): string {
  if (section.status === "available") return "evidence валідний";
  const reason = text(section.reason);
  return (
    (
      {
        not_configured: "джерело не налаштоване",
        unsupported_metric: "метрика не підтримується",
        boundary_evidence_missing: "немає boundary evidence",
        boundary_evidence_stale: "boundary evidence застарілий",
        invalid_evidence: "evidence невалідний",
        counter_discontinuity: "розрив лічильника",
        continuity_gap: "розрив безперервності",
      } as Record<string, string>
    )[reason ?? ""] ?? "evidence недоступний"
  );
}
function duration(section: JsonRecord): string {
  const seconds = finite(section.duration_seconds);
  if (section.status !== "available" || seconds === null || seconds < 0) return "Недоступно";
  const minutes = seconds / 60;
  return Number.isInteger(minutes) ? `${minutes} хв` : `${minutes.toFixed(1)} хв`;
}
function derivedMetric(value: unknown, unit: string): string {
  const section = record(value);
  if (section.status !== "available") return "Недоступно";
  const n =
    finite(section.value) ??
    finite(section.value_c) ??
    finite(section.temperature_c) ??
    finite(section.value_k) ??
    finite(section.delta_k);
  return n === null ? "Недоступно" : `${n.toFixed(1)} ${text(section.unit) ?? unit}`;
}
function unavailableReason(reason: string | null): string {
  return (
    (
      { no_data: "немає даних", stale: "застарілі дані", invalid_quality: "невалідна якість" } as Record<
        string,
        string
      >
...
function severityLabel(value: string): string {
  return (
    ({ critical: "Критично", warning: "Увага", info: "Інформація" } as Record<string, string>)[value] ??
    "Тривога"
  );
}
function qualityReason(value: string): string {
  return (
    (
      {
        m_packet_coverage_incomplete: "неповне покриття M-пакетів",
        compressor_coverage_incomplete: "неповне покриття компресора",
        energy_evidence_unavailable: "energy evidence недоступний",
        defrost_evidence_unavailable: "відтайка недоступна",
        defrost_coverage_incomplete: "неповне покриття відтайки",
      } as Record<string, string>
    )[value] ?? "частина evidence недоступна"
  );
}
...
function formatHours(minutes: number): string {
  ...
  return `${Number.isInteger(hours) ? hours : hours.toFixed(1)} год`;
}
