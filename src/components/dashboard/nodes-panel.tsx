...
import { edgeNodes } from "@/data/dashboard";
      ...
      {edgeNodes.map((node) => (
        <article key={node.id} className="group flex items-center gap-3 py-3">
          <div className="relative grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-white/[0.075] bg-white/[0.025] text-slate-400 transition group-hover:border-blue-400/25 group-hover:text-cyan-300">
            ...
            <span
              className={`absolute right-1 bottom-1 h-2 w-2 rounded-full border-2 border-[#0b213f] ${node.state === "warning" ? "bg-amber-400" : "bg-emerald-400"}`}
            />
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-[8px] font-medium ${node.state === "warning" ? "text-amber-400" : "text-emerald-400"}`}
              >
                {node.state === "standby" ? "Standby" : node.state === "warning" ? "Warning" : "Online"}
              </span>
            </div>
          ...
          <div className="w-[68px] shrink-0 space-y-1 text-[8px] text-slate-500">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-slate-300">{node.ram}%</span>
            </div>
          </div>
        </article>
      ))}
