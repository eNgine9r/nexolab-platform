...
export function CamerasWorkspace({ items, rejected }: { items: CameraRecord[]; rejected: number }) {
  ...
  const onlineCount = items.filter((item) => item.state === "online").length;
  ...
  return (
    <section className="space-y-4" aria-labelledby="cameras-title">
      <header className="rounded-3xl border border-white/10 bg-[#091a31]/95 p-5 shadow-2xl shadow-black/20">
        <p className="text-xs tracking-[0.2em] text-cyan-300 uppercase">Local camera readiness</p>
        <h1 id="cameras-title" className="mt-2 text-2xl font-semibold text-white">
          Камери
        </h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
          Лише перевірений локальний inventory і чесні стани доступності. Статична конфігурація або декоративне зображення не вважаються доказом LIVE.
        </p>
      </header>
      ...
      <div className="grid gap-3 rounded-2xl border border-white/10 bg-[#091a31]/80 p-4 md:grid-cols-[1fr_240px]">
        ...
        <label>
          ...
          <select
            value={state}
            onChange={(event) => setState(event.target.value as "all" | CameraState)}
      ...
      {items.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-cyan-300/20 bg-[#091a31]/70 p-8 text-center">
          ...
          <h2 className="mt-4 text-lg font-semibold text-white">Камери не налаштовані</h2>
          <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-400">
            У репозиторії немає перевіреного camera inventory або безпечного media gateway. NEXOLAB не показує вигадані LIVE-потоки й не відкриває raw RTSP у браузері.
          </p>
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((camera) => (
            <article key={camera.id} className="rounded-2xl border border-white/10 bg-[#091a31]/90 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <h2 className="truncate font-medium text-white">{camera.name}</h2>
              ...
              <dl className="mt-4 space-y-2 text-sm">
                ...
                <Row label="Endpoint" value={camera.endpoint ?? "Приховано або відсутній"} />
              </dl>
              {camera.reason ? (
...
function Metric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl border border-white/10 bg-[#091a31]/80 p-4"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold text-white">{value}</p></div>;
}

function Row({ label, value }: { label: string; value: string }) {
  return <div className="grid grid-cols-[90px_1fr] gap-3"><dt className="text-slate-500">{label}</dt><dd className="min-w-0 break-all text-slate-300">{value}</dd></div>;
}
