...
export function CamerasWorkspace({ items, rejected }: CamerasWorkspaceProps) {
    ...
    <section className="space-y-4" aria-labelledby="cameras-title">
      <header className="rounded-3xl border border-white/10 bg-[#091a31]/95 p-5 shadow-2xl shadow-black/20">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
          Лише перевірений локальний inventory і чесні стани доступності. Статична конфігурація або
          декоративне зображення не вважаються доказом LIVE.
        </p>
      </header>
      ...
      {items.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-cyan-300/20 bg-[#091a31]/70 p-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-400">
            У репозиторії немає перевіреного camera inventory або безпечного media gateway. NEXOLAB не показує
            вигадані LIVE-потоки й не відкриває raw RTSP у браузері.
          </p>
        </div>
      ) : filtered.length === 0 ? (
