...
export function UsersScreen() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setSelectedId((current) =>
        current && nextUsers.some((user) => user.id === current) ? current : (nextUsers[0]?.id ?? null),
      );
    } catch (cause) {
    ...
    return users.filter((user) => {
      const searchMatch =
        ...
        [user.username, user.displayName ?? "", user.email ?? ""]
          ...
          .includes(normalized);
      const roleMatch =
        roleFilter === "all" || (roleFilter === "legacy" ? user.migrationRequired : user.role === roleFilter);
      const activeMatch =
  ...
  if (security.mode === "demo") {
    return (
      <UsersGate
        title="Користувачі доступні лише в local live mode"
        message="Users & Access працює з локальною PostgreSQL identity authority і не використовує demo accounts."
      />
    );
  }
  ...
  if (!security.session || !security.membership) {
    return (
      <UsersGate
        ...
        message="Для керування користувачами потрібне активне локальне membership."
      />
    );
  }
  ...
  if (!client) {
    return (
      <UsersGate
        ...
        message="Не вдалося визначити локальний Telemetry Service для Users & Access."
      />
    );
  }
              ...
              <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">
                      Чотири продуктові ролі. Адміністратор має повний доступ; для керівника лабораторії,
                      інженера та техніка-лаборанта права задаються явно й перевіряються сервером.
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-sm hover:border-cyan-300/30 disabled:opacity-50"
                  >
              ...
              <section className="rounded-3xl border border-blue-400/20 bg-[#091a31] p-5">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <Field label="Логін">
                    <input
                      ...
                      onChange={(event) =>
                        setCreateDraft((draft) => ({ ...draft, username: event.target.value }))
                      }
                      autoComplete="off"
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setCreateDraft((draft) => ({
                          ...draft,
                          role: event.target.value as SecurityProductRole,
                          permissions: event.target.value === "administrator" ? [] : draft.permissions,
                        }))
                      }
                      className={inputClass}
                    >
                ...
                {createDraft.role !== "administrator" ? (
                  ...
                  <p className="mt-4 text-sm text-emerald-300">
                    Адміністратор автоматично отримує повний каталог дозволів.
                  </p>
                )}
                <div className="mt-4 flex justify-end">
                  <button
                    type="button"
                    disabled={busy || !createDraft.username.trim() || createDraft.password.length < 12}
                    onClick={() => void createUser()}
                    className="rounded-xl bg-blue-500 px-4 py-2.5 text-sm font-medium disabled:opacity-50"
                  >
                    Створити
                  </button>
                </div>
              ...
              <div className="rounded-3xl border border-white/10 bg-[#091a31]/90 p-4">
                <div className="grid gap-2 sm:grid-cols-3">
                  ...
                  >
                    ...
                    <option value="legacy">Потребує міграції</option>
                  </select>
                  <select
                ...
                <div className="mt-4 space-y-2">
                  {loading ? (
                    <p className="p-4 text-sm text-slate-400">Завантаження локальних користувачів…</p>
                  ) : null}
                  {!loading && filteredUsers.length === 0 ? (
                    <p className="p-4 text-sm text-slate-400">Користувачів за цими умовами не знайдено.</p>
                  ) : null}
                  {filteredUsers.map((user) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span
                          className={`rounded-full px-2 py-1 text-[11px] ${user.isActive ? "bg-emerald-400/10 text-emerald-300" : "bg-slate-500/10 text-slate-400"}`}
                        >
                ...
                {!selectedUser ? (
                  ...
                  <div className="space-y-5">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <p className="text-xs tracking-[0.18em] text-cyan-300 uppercase">Access profile</p>
                        <h2 className="mt-1 text-xl font-semibold">
                          {selectedUser.displayName || selectedUser.username}
                        </h2>
                        <p className="mt-1 text-sm text-slate-500">{selectedUser.username}</p>
                      </div>
                      {selectedUser.migrationRequired ? (
                        <div className="max-w-sm rounded-xl border border-amber-400/20 bg-amber-400/[0.06] p-3 text-xs leading-5 text-amber-200">
                          <AlertTriangle className="mr-2 inline h-4 w-4" />
                          Старий role value збережено. Виберіть одну з чотирьох продуктових ролей, щоб
                          завершити контрольовану міграцію.
                        </div>
                      ) : null}
                    </div>
                    ...
                    <Field label="Роль">
                      <select
                        value={selectedRole}
                        onChange={(event) => {
                          const role = event.target.value as SecurityProductRole;
                          setSelectedRole(role);
                          if (role === "administrator") setSelectedPermissions([]);
                        }}
                        className={inputClass}
                      >
                        {(roles.length ? roles : fallbackRoles()).map((role) => (
                    ...
                    {selectedRole === "administrator" ? (
                      <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.06] p-4 text-sm text-emerald-200">
                        ...
                        Повний доступ, включно з керуванням користувачами та permission boundary для керування
                        версіями NEXOLAB.
                      </div>
                    ) : (
                      <PermissionGrid
                    ...
                    <div className="flex flex-wrap gap-2 border-t border-white/8 pt-4">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-xl bg-blue-500 px-4 py-2.5 text-sm font-medium disabled:opacity-50"
                      >
                        ...
                        onClick={() => {
                          if (
                            !window.confirm(
                              selectedUser.isActive
                                ? "Деактивувати користувача та відкликати його сесії?"
                          ...
                          void runMutation(async () => {
                            await client.updateUser(selectedUser.id, {
                              ...
                              reason: selectedUser.isActive
                                ? "Deactivated from Users & Access"
                                : "Activated from Users & Access",
                            });
                            setNotice(
                              selectedUser.isActive ? "Користувача деактивовано." : "Користувача активовано.",
                            );
                          });
                        }}
                      ...
                      >
                        ...
                        {selectedUser.isActive ? "Деактивувати" : "Активувати"}
                      </button>
                      <button
                        type="button"
                        disabled={busy}
                        onClick={() => {
                          if (!window.confirm("Відкликати всі активні сесії цього користувача?")) return;
                          void runMutation(async () => {
                            const count = await client.revokeSessions(
                              selectedUser.id,
                              "Revoked from Users & Access",
                            );
                            setNotice(`Відкликано сесій: ${count}.`);
                          });
                        }}
                        className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-sm disabled:opacity-50"
                      >
                    ...
                    <div className="rounded-2xl border border-white/8 bg-white/[0.025] p-4">
                      ...
                      <p className="mt-1 text-xs leading-5 text-slate-500">
                        Новий пароль одразу хешується на backend. Після операції всі активні сесії користувача
                        відкликаються.
                      </p>
                      <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setResetPassword("");
                            if (!window.confirm("Скинути пароль і відкликати всі сесії?")) return;
                            void runMutation(async () => {
                              await client.resetPassword(
                                selectedUser.id,
                                password,
                                "Password reset from Users & Access",
                              );
                              setNotice("Пароль змінено; активні сесії відкликано.");
                            });
                          }}
                          className="inline-flex items-center justify-center gap-2 rounded-xl border border-cyan-300/20 px-4 py-2.5 text-sm disabled:opacity-50"
                        >
...
function PermissionGrid({
  ...
  options: LocalUserPermissionOption[];
  selected: SecurityEffectivePermission[];
  onToggle: (permission: SecurityEffectivePermission) => void;
}) {
  return (
    <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
      {options.map((option) => (
        <label
          key={option.value}
          className="flex cursor-pointer items-start gap-3 rounded-xl border border-white/8 bg-white/[0.025] p-3 text-sm"
        >
          <input
            type="checkbox"
          ...
          />
          <span>
            <span className="block text-slate-200">{permissionLabels[option.value] ?? option.value}</span>
            <span className="mt-1 block text-[11px] text-slate-500">{option.value}</span>
          </span>
        </label>
      ))}
    </div>
...
function Notice({ tone, text }: { tone: "error" | "success"; text: string }) {
  return (
    <div
      className={`rounded-2xl border px-4 py-3 text-sm ${tone === "error" ? "border-rose-400/20 bg-rose-400/[0.06] text-rose-200" : "border-emerald-400/20 bg-emerald-400/[0.06] text-emerald-200"}`}
    >
      {text}
    </div>
  );
...
function UsersGate({ title, message }: { title: string; message: string }) {
  return (
    <main className="grid min-h-screen place-items-center bg-[#06142a] p-4 text-slate-100">
      <section className="w-full max-w-lg rounded-3xl border border-cyan-400/15 bg-[#091a31]/95 p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs tracking-[0.2em] text-cyan-300 uppercase">NEXOLAB Users & Access</p>
...
function fallbackRoles(): LocalUserRoleOption[] {
  return (Object.entries(roleFallbackLabels) as [SecurityProductRole, string][]).map(([value, label]) => ({
    value,
    label,
    fullAccess: value === "administrator",
    permissionsEditable: value !== "administrator",
  }));
}
...
function togglePermission(
  current: SecurityEffectivePermission[],
