...
**Ready for read-only preflight; production mutation remains a hard gate.** The currently deployed persistent Gateway image `sha256:16a78adc…` is topic-aware but behaviorally fails the new recurring-activation boundary by discovering an unapproved pre-cutoff synthetic snapshot. #901 must pin exact current `main`, prepare/prove a boundary-aware image, fingerprint the existing two-row sent outbox, preserve core container/Tailscale identities, and prepare rollback evidence while delivery and scheduler stay OFF. Only after that read-only preflight may an explicit Product Owner approval authorize the Gateway-only production recreate. #901 does not authorize a Telegram send or recurring activation.
