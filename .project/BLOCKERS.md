...
**Active metadata-only blocker.** Product Owner-approved Stage-2 deployment passed on exact source `13ab26392fcb1c1385dca3c1f619da4512fe568c` with authoritative evidence `runtime/deployments/20260903T191159Z`; live Alembic is `20260902_0031`. Runtime is healthy, persistent volumes are preserved, Telegram delivery/worker remain OFF, Mini App remains ON, `last_send_at=null`, and `DAILY_REPORTS_SCHEDULER_ENABLED=false`.

`/var/lib/nexolab/version-management/current.json` still records prior source `893573a86faf6629e5e3cefc0e41c47230a7f8c7` / schema `20260902_0030`. The canonical `scripts/nexolab-adopt-source-deployment.py` must reconcile that metadata against the successful Stage-2 evidence. The adopter is runtime/database/hardware read-only and writes only version-management metadata, but requires privileged access. Do not hand-edit `current.json`; #851 must remain incomplete until the adopter succeeds and the result is verified.

## Issue #825 — TG-04 real Telegram acceptance

**Stage-2 deployment cleared; temporarily Ready behind #851 reconciliation.** Exact source `13ab26392fcb1c1385dca3c1f619da4512fe568c` deployed successfully using the verified ARM64 candidate. Live schema is `20260902_0031`; Dashboard, Telemetry, MQTT and Device Agent are healthy. Telegram Gateway is `ready` with `delivery_enabled=false`, `miniapp_enabled=true`, `running=false`, `last_send_at=null`; the 07:50 scheduler remains explicitly disabled. Tailscale `:8444` proxies the deployed Dashboard on `:3000`. No TestLAB report was sent during Stage 2.

After #851 source-adoption reconciliation, resume #825 with signed Mini App persisted-report acceptance. One controlled TestLAB send, reliability/no-duplicate evidence and Monday-Friday 07:50 `Europe/Kyiv` activation remain later gates and must not be conflated with the completed Stage-2 cutover.
