...
**Active repository prerequisite.** The topic-aware persistent Gateway refresh and restart/no-duplicate acceptance are PASS. Recurring scheduler/delivery remain OFF. #898 adds a repository-owned two-phase activation guard: exact read-only planner → scheduler-only reconciliation → exact snapshot delta proof → protected phase-two boundary → topic worker enablement → exact approved delivery delta proof. PR #900 already hardened active observability/local-auth preservation, exact persistent rollback restoration, shared production-mutation locks and `O_NOFOLLOW` same-descriptor user locks. After exact head `8c4c123e4db5ac9d0d8dd17ffa0dc604c1609356` passed Core Quality/build, Telemetry Service and NEXOLAB Merge Gate, fresh review correctly reopened merge with two P1 findings and one P2: an eligible snapshot could be inserted after the phase-two plan capture, rollback could wait on Telemetry while an enabled Gateway continued outbound work, and planner runtime subprocesses lacked a timeout. The current local remediation acquires a PostgreSQL `SHARE` table fence on `refrigeration_daily_report_snapshots`, revalidates the exact plan after the fence is authoritative, keeps snapshot writes blocked through exact delivery acceptance, bounds that fence to 600 seconds with a 120-second delivery convergence budget, quiesces/stops the Gateway before any Telemetry rollback wait, and gives every planner runtime read a 15-second timeout. The updated TG-04 recurring/Gateway-refresh/Stage-1 safety cascade is 41/41 PASS; live read-only evidence still predicts one 2026-09-04 catch-up snapshot and exactly one immediate topic delivery with two historical sent rows, zero non-sent rows and zero duplicate-risk rows. These latest review fixes are still local and require a new immutable candidate, canonical exact-candidate GREEN, push, review resolution and fresh exact-head CI before merge. Production activation remains a separate Product Owner hard gate after merge and a fresh preflight.

**Newest review status.** Post-`276874c6` review added two additional P1 findings: a snapshot writer queued behind the phase-two fence could commit after release and be discovered by the enabled worker, and rollback could continue toward Telemetry when Gateway quiescing remained unproved. Both are remediated locally with an exact bootstrap snapshot-ID allow-set plus activation cutoff and a strict Gateway-quiesce gate that aborts Telemetry rollback on failure. Latest focused evidence is 43/43 TG-04 safety tests, 87/87 Gateway tests and disconnected candidate-image health. No production flag changed; scheduler and delivery remain OFF. The remediation is now frozen in the local candidate; the repository blocker remains until canonical exact-candidate verification is GREEN, the exact head is pushed, reviewed and remote exact-head checks are GREEN.

**Latest exact-head P1.** Codex review of `81cf16c5` proved the currently deployed Gateway image predates the new cutoff/bootstrap worker contract, so reusing that image would ignore the boundary even though the env values are present. Local remediation adds a pre-mutation, network-isolated capability proof against the exact pinned current image. The current production image fails that proof as expected; the boundary-aware candidate image passes. This makes recurring activation fail closed until a separate Product Owner-approved Gateway-only refresh has installed a boundary-aware image with scheduler/delivery still OFF. The recurring guard itself does not upgrade the image. Focused TG-04 safety cascade is now 44/44 PASS. Repository blocker remains canonical exact-candidate GREEN → push → resolve this P1 → fresh exact-head GREEN/review → merge. After merge, production Gateway refresh is a separate hard gate before recurring activation.

**Fresh `28b427c9` review P1.** Codex correctly rejected source-token matching as sufficient capability proof. The local guard now behaviorally exercises packaged `_discover()` inside the exact pinned image under `--pull never --network none --read-only` with an approved pre-cutoff snapshot, an unapproved pre-cutoff snapshot and a due post-cutoff snapshot. Candidate `sha256:3c34a4e1…` returns only the approved + post-cutoff identities and passes; deployed `sha256:16a78adc…` returns all three and fails, proving the old image cannot cross the activation gate. The probe has no Telegram API call and no persistent write. TG-04 safety is 44/44 PASS and Gateway is 87/87 PASS. Repository blocker is now only new immutable candidate → canonical GREEN → push → resolve fresh P1 → fresh exact-head CI/Merge Gate/review. Production refresh/activation remain separate Product Owner hard gates.
