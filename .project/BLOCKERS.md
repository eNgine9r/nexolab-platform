...
RFX-01 / #717 and RFX-02 / #935 are completed through GREEN PRs #934 and #936. RFX-03 / #937 remains the active software Work Package on `feat/937-rfx03-humidity-acquisition`, based on merged main `6b63518f...`. Three fresh Codex reviews across `683aaea...`, `de8140c6...` and `4382d9fd...` found four related P2s around source/profile compatibility: stale linked profile after rebinding, a generic-evaluator bypass, unlinked-profile scaling after source history exists, and a cross-session concurrent-rebind race. All four are now locally remediated: explicit profile→source provenance remains enforced, generic legacy unlinked scaling is allowed only for Signals with no source history, and source/profile evaluation occurs in one transaction under the owning Signal lock before scaling. Post-third-review non-PostgreSQL evidence is GREEN at `45 passed / 12 PostgreSQL-only skipped`; a PostgreSQL concurrency regression has been added but is intentionally not executed in a new local container on the resource-constrained production Pi. Exact head `4382d9fd...` had GREEN Telemetry PostgreSQL/migration/round-trip and Core Quality/build before the latest repository/test change, so those results are superseded as merge authority and must rerun on the next exact head. Its Authenticated Dashboard failure was isolated to one socket-count assertion with 19/20 tests passing and prior exact-head successes in the same PR; the next exact head must nevertheless return that routed workflow GREEN. Exact head `81886974...` subsequently failed Telemetry only in the newly added PostgreSQL concurrency test harness: the run reached `641 passed / 1 failed`, with `writer_repository` undefined before either concurrency thread started. Fresh Codex review raised the same defect as the sole new P1. The harness is locally corrected by instantiating the dedicated writer repository in that test scope; product repository logic is unchanged. New exact-head PostgreSQL CI, all routed workflows, NEXOLAB Merge Gate and a fresh Codex review with no unresolved P1/P2 are required before merge. #189/#585 remain blocked, #201 remains `needs_validation`, and #202 remains `hardware_validation`.
