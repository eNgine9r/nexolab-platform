...
The first real human identity-link helper run failed before credential entry because the host system Python lacked Pydantic transitively imported through `app.miniapp`. This is a repository/runtime defect, not an external-user blocker. A stdlib-only validator repair is locally verified with host-system-Python CLI startup, `5/5` focused tests, `51/51` full Telegram Gateway and isolated compile. Current gate: exact-head CI for the repair. Only after GREEN does the Product Owner rerun the documented private challenge. BotFather HTTPS, phone WebView, production deployment/test-send and schedule gates remain later boundaries. No Modbus/hardware write or destructive data action is authorized.
