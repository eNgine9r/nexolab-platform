...
Issue #847 state reconciliation is GREEN and TG-04 / #825 is active again. The current-main ARM64 build `33781986481` / artifact `9904235935` is GREEN but intentionally rejected as the Stage-2 candidate because it omits TG-04-only repairs still carried by draft PR #842. Current `main` is being integrated into `ops/825-telegram-real-acceptance`; the merged PR head must pass exact-head CI and receive a matching ARM64 artifact. After those software gates, the remaining hard boundary before Stage-2 execution is the separate explicit Product Owner approval for controlled production cutover. No report has been sent and the 07:50 schedule remains disabled/not deployed. Modbus/hardware writes, destructive data operations and volume deletion remain forbidden.
