...
**Repository gate in progress; production mutation remains a later hard gate.** PR #904 hardens the Gateway refresh so approval is bound to one immutable prebuilt image with exact OCI source revision/tree and a behavioral boundary probe before mutation. A provisional pre-merge proof on `d6d3332bc43764831cc27b3a8c43e731965df87e` produced `sha256:83aa8f8e82cf812d6bb0c8f6970b0cb6818cdf0574d8cb1b01689e3249d22477` and PASSed the boundary probe while deployed `sha256:16a78adc648ddc84a74d3098e8bb8338d3cd5349c6a055b801dc93ee675447c6` FAILed as expected; this remains non-authoritative post-merge evidence only. Fresh review then found three approval-authority defects across successive candidate heads: mutable working-tree build context, stale `origin/main` authority, and no standalone pre-approval boundary command. All three are now remediated locally: candidate build context streams the exact Git object subtree and stamps/verifies its tree; approval preparation fast-forwards freshly fetched `origin/main`; the guarded refresh independently fetches the remote tip; and `scripts/telegram-gateway-boundary-runtime-proof.sh` supplies the exact no-secret, no-production-mount, `--network none --read-only` proof and is reused by the guard before `MUTATED=1`. Focused TG-04 safety is 47/47 PASS and a real helper proof against pre-merge source `5b180658...` / image `sha256:24823ade...` is PASS. Full local canonical verification of final repository code candidate `dda39db7b9ddd1486163975a8a483eb50e2c1eb6` is GREEN (including 143/143 frontend files, 697/697 tests, production build and central/edge Compose). A state-only checkpoint delta is the only remaining local change before push. These local results are not merge authority: require fresh exact-head GitHub CI/Codex review, resolve all review threads, then merge #904 only GREEN. After merge, rebuild from exact freshly synced merged `main`, rerun the full read-only outbox/core/Tailscale/rollback preflight, and only then request explicit Product Owner approval for Gateway-only production recreation. Delivery and scheduler remain OFF; no Telegram send or recurring activation is authorized.

## Issue #825 — TG-04 real Telegram acceptance

**Blocked on #901 after repository guard completion.** Real signed iOS Mini App acceptance, General one-shot, protected topic capture, topic dry-run, forum-topic one-shot, the earlier topic-aware persistent Gateway refresh, and restart/no-duplicate acceptance are PASS. Issue #898 is now repository-complete. The next prerequisite is #901: install a boundary-aware persistent Gateway with delivery OFF and scheduler OFF. After #901 refresh acceptance, run a fresh read-only recurring activation preflight and request a separate Product Owner activation approval using the then-current exact immediate-delivery count.

Independent durable outbox baseline remains exactly two historical `sent` rows (General + forum topic), both `attempts=1`, both message IDs present, `non_sent=0`, `duplicate_risk=0`, and no unknown-delivery recovery. No automatic resend is authorized.
