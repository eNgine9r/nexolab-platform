...
## RFX-08B / Issue #960 — active; no software hard blocker

#960 has locally implemented the audited authority boundary without requiring a new Product Owner threshold decision: explicit immutable versioned policies are persisted with no seeded production values, generic digital/analog acquisition-profile `acceptance-state/v1` is independent from Instrument acceptance, and historical derived reads use raw `telemetry_samples` rather than `telemetry_latest`. Migration `20260907_0036` passed isolated PostgreSQL upgrade/downgrade/re-upgrade and direct guard tests; full affected refrigeration/instrumentation PostgreSQL matrices are GREEN. The remaining gate is software-only: final pre-commit review, exact committed-head verification, focused PR and exact-head CI. Final local verification is GREEN through 224 passed / 1 skipped on the affected PostgreSQL matrix after lifecycle-first and UUID-independent Instrument-version replay hardening. Production runtime remains unchanged; no Modbus write, hardware write, production cutover or hardware acceptance is authorized.
