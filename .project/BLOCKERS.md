...
**Software/Stage-1/pre-cutover gates cleared; production cutover remains a hard approval gate.** Dedicated bot/group identity, protected secrets, least-privilege backend principal, administrator Telegram identity link, tailnet HTTPS and real iOS WebView are confirmed. Stage 1 gateway is healthy with `delivery_enabled=false`, `miniapp_enabled=true`, worker `running=false` and no send. Scheduler prerequisite #845 is merged with fail-closed default OFF. TG-04 PR #842 is merged to `main` at `13ab26392fcb1c1385dca3c1f619da4512fe568c`; Issue #825 remains open.

Exact-main ARM64 artifact run `33786462681` / artifact `9905912342` is GREEN and staged at `runtime/staging/tg04-stage2-13ab2639`; internal artifact checksums/provenance match exact main, `linux/arm64`, Node `22.23.1` and the accepted LAN/local-auth public contract. Source-selection check-only is GREEN. Capacity preflight passed without pruning (`11065131008` free / `9549148448` required / `2147483648` reserve), including streamed `pg_dump` sizing. Production remains source `893573a86faf6629e5e3cefc0e41c47230a7f8c7`, schema `20260902_0030`; the target adds only daily-report migration `20260902_0031`. There is no Device Agent, RS-485, Modbus or hardware configuration delta. Dashboard/Telemetry/Mini App remain healthy and Telegram delivery/schedule remain OFF.

Issue #849 first exact state head `1b83981c80e11e097b0566842f074802fba65ac4` passed State Integrity and NEXOLAB Merge Gate in CI run `33792778506`; its completion candidate restores #825 active and now requires only final PR #850 state-head re-verification/merge. After that merge, **stop for separate explicit Product Owner approval** before any Stage-2 production/site cutover. Approval is not implied by prior manual actions. No controlled deployment, migration, report send, schedule enablement, Modbus/hardware write or destructive data/volume operation may occur before that approval.
