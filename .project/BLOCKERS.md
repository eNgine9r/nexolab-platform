...
The protected-host provisioning path is now implemented and locally verified (`5/5` focused provisioning tests, `46/46` full Telegram Gateway, isolated compile). It authenticates through the audited local-auth/admin APIs, creates `nexolab-telegram` only when absent with `laboratory_technician + reports.read`, never prints admin/backend credentials or tokens, preserves interrupted credentials through a root-only pending file, and writes `telegram.env` with both Telegram switches disabled. The assistant cannot execute this step because it requires an existing administrator credential and root-owned production security/runtime writes. Required Product Owner action: run the documented `app.runtime_provisioning` helper locally and return only its sanitized JSON result. BotFather Main Mini App HTTPS configuration, real authorized Telegram-user identity linking, phone/Tailscale WebView acceptance, and the separately approved production deployment/test-send/schedule gates remain later boundaries. No Modbus/hardware write or destructive data action is authorized.
