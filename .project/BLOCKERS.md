...
RFX-01 / #717 and RFX-02 / #935 are completed through GREEN PRs #934 and #936. RFX-03 / #937 is the active software Work Package on `feat/937-rfx03-humidity-acquisition`, based on merged main `6b63518f...`. Codex found two source/profile compatibility P2s: exact head `683aaea...` lacked an explicit profile→source compatibility boundary after rebinding, and exact head `de8140c6...` showed the generic `/analog-scaling-evaluate` path could bypass the specialized humidity guard. Both are locally remediated: profiles can carry explicit acquisition-source provenance, and every source-linked profile is now checked against the effective source in the shared RFX-02 evaluator before scaling; humidity additionally requires an explicit matching link. Post-second-remediation non-PostgreSQL evidence is GREEN at `44 passed / 11 PostgreSQL-only skipped` plus humidity regression `6/6`; migration head remains `20260906_0034`, last offline PostgreSQL SQL generation is `2842` lines, and the automated PostgreSQL migration round-trip test remains included. Previous local `10/10` PostgreSQL and manual round-trip evidence is pre-first-remediation only; a new exact-head commit, PostgreSQL CI, routed workflows, Merge Gate and fresh Codex review are required before merge. #189/#585 remain blocked, #201 remains `needs_validation`, and #202 remains `hardware_validation`.
