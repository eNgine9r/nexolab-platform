...
The protected-host provisioning path is exact-head GREEN at `b26e51ae23022a887c38ae3a2008392710aff7da` across Core/Merge Gate, Telegram Gateway, Telemetry, Container Supply Chain and Offline Bundle. It authenticates through audited local-auth/admin APIs, creates `nexolab-telegram` only when absent with `laboratory_technician + reports.read`, never prints admin/backend credentials or tokens, preserves interrupted credentials through a root-only pending file, and writes `telegram.env` with both Telegram switches disabled. A separate explicit human identity-link helper is locally verified (`4/4` focused, `50/50` combined gateway, isolated compile): it requires an existing active NEXOLAB user with `reports.read`, accepts only a fresh one-time command sent in the bot's private chat, stores the Telegram user id only in root-owned `identity-links.json`, and fails closed on conflicts. Its exact-head CI is still pending. The assistant cannot execute either protected-host step because they require an existing administrator credential and root-owned production security/runtime writes. After the new exact head is GREEN, required Product Owner action is to run the documented `app.runtime_provisioning` and then `app.identity_link_provisioning` helpers locally, returning only sanitized results. BotFather Main Mini App HTTPS configuration, phone/Tailscale WebView acceptance, and separately approved production deployment/test-send/schedule gates remain later boundaries. No Modbus/hardware write or destructive data action is authorized.
