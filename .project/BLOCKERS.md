...
**Hard-gated on separate recurring activation approval.** Real signed Mini App acceptance, General one-shot, protected forum-topic capture, topic dry-run, forum-topic one-shot, guarded recurring-activation tooling, and the boundary-aware persistent Gateway refresh are all PASS. Fresh read-only planner evidence at 2026-09-05 06:51 Europe/Kyiv reports latest due date `2026-09-04`, no due snapshot yet, predicted snapshot generation count `1`, predicted immediate delivery count `1`, outbox rows `2`, non-sent `0`, duplicate-risk `0`, and runtime configuration ready. The next step is an explicit Product Owner approval for recurring weekday 07:50 activation with the current approved immediate-delivery count. The production guard must replan immediately before mutation and fail closed if the count or pinned runtime authority changes. Until that approval is consumed, Telegram delivery and the scheduler remain OFF and no report send is authorized.
