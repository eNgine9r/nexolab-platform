...
**Active repository prerequisite.** The topic-aware persistent Gateway refresh and restart/no-duplicate acceptance are PASS. Recurring scheduler/delivery remain OFF. #898 adds a repository-owned two-phase activation guard: exact read-only planner → scheduler-only reconciliation → exact snapshot delta proof → topic worker enablement → exact approved delivery delta proof. PR #900 review hardening requires the controlled observability overlay and active local-auth overlay to survive both forward and rollback Telemetry recreations; rollback cannot report PASS unless both persistent env files are restored and revalidated OFF, Telemetry recreation succeeds with runtime `DAILY_REPORTS_SCHEDULER_ENABLED=false`, and the root-only backup is retained if persistent restore cannot be proved. The guard also holds the existing Gateway-refresh, Stage-1 and both canonical current-head deployment locks before its locked runtime baseline. The latest PR #900 P1 concurrency/security review is remediated locally: invoking-user current-head locks are opened with `O_NOFOLLOW`, validated and flocked on the same descriptor after dropping privileges to the sudo UID/GID, while root mutation lock paths reject symlink/user-owned/unsafe-parent states before shell flock acquisition; lock-holder loss/zombie state fails closed before mutation. Current read-only planner evidence still predicts one 2026-09-04 catch-up snapshot and exactly one immediate topic delivery; this is not authorization to send it. The current broader TG-04 safety cascade is 38/38 PASS with Compose overlay validation and the live planner still clean. Production activation remains a separate Product Owner hard gate after final exact-head GREEN and merge.
