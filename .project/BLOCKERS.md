...
Issue #994 is in review. Fresh no-cache discovery run `34699985764` identified exactly four stale Telemetry Service exceptions and implementation commit `7fb27f19cd7a9a8a48f4bcc0ce74def7f7d760d2` removed them. After #997 / PR #998 restored the unrelated Telemetry MinIO CI fixture, exact-head no-cache run `34702355254` found one new HIGH tuple, `telemetry-service/libcjson1/CVE-2026-87933`. Current package/symbol evidence shows the vulnerable cJSON Utils MergePatch library is not linked or referenced by `mosquitto_ctrl`, and NEXOLAB exposes no arbitrary Merge Patch input. Late-triage commit `be925a074780cb5fc7ac09b551f26e3152e1fcfb` is the canonical verified implementation head and adds one exact short-lived exception, so the candidate retains 92 exact HIGH entries only through `2026-09-15`. Current local validator, both focused supply-chain suites (33/33), Python compile, lint-staged/Prettier, State Model v2 and diff hygiene are GREEN. The remaining software gate is fresh exact-head Container Supply Chain, Telemetry Service, Core CI and `NEXOLAB Merge Gate`; this is not a production runtime change. #189/#585 remain blocked, #201 remains `needs_validation`, #202 remains `hardware_validation`, and K96–K100 remain hardware-unverified/excluded from automatic production polling.
