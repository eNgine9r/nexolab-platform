...
**Repository gate in progress; production mutation remains a later hard gate.** PR #904 hardens the Gateway refresh so approval is bound to one immutable prebuilt image with exact OCI source revision/tree and a behavioral boundary probe before mutation. Fresh Codex review found a P1 race in the documented working-tree target build; remediation now streams the exact Gateway subtree from the Git object database with `git archive`, stamps `io.nexolab.source-tree`, and requires the refresh guard to match that tree label to the exact expected Git tree before behavioral probing or mutation. Focused TG-04 safety is 46/46 PASS. A provisional pre-merge proof on `d6d3332bc43764831cc27b3a8c43e731965df87e` produced `sha256:83aa8f8e82cf812d6bb0c8f6970b0cb6818cdf0574d8cb1b01689e3249d22477` and PASSed the new network-none/read-only capability probe while deployed `sha256:16a78adc648ddc84a74d3098e8bb8338d3cd5349c6a055b801dc93ee675447c6` FAILed as expected; no-approval exit 64 and no-production-mutation evidence are also PASS in `runtime/evidence/tg04-901-preflight-20260904T211013Z`. This image is not final approval authority because PR #904 has not yet merged. First merge PR #904 only after exact-head GREEN CI/review. Then rebuild from exact merged `main`, rerun the read-only outbox/core/Tailscale/rollback preflight, and only then request explicit Product Owner approval for the Gateway-only production recreate. Delivery and scheduler remain OFF; no Telegram send or recurring activation is authorized.
