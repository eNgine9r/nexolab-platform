...
Issue #965 is still the active verification gate. Local implementation candidate `bf0d14135bcdc71cc73a92e4b149a3aea60cd0b2` now addresses both newest review findings: the open quick picker refreshes live channel samples on the existing LOCAL_LAN API while retaining a freshness clock, and saved automatic inventory labels recover their provenance across reload plus delayed catalog enrichment before channel replacement. Focused source-tree tests are 30/30 PASS; targeted ESLint, Prettier, source-only TypeScript and whitespace checks are GREEN. The raw Pi typecheck is not authoritative because ignored historical `runtime/**` release/deployment source trees are included by the broad root tsconfig. Merge remains gated on fresh exact-head GitHub CI, Refrigeration Browser acceptance, production build and a clean final review. This is a verification gate, not a product/hardware blocker. K96–K100 remain hardware-unverified and excluded from automatic production polling. #189/#585 remain blocked, #201 remains `needs_validation`, and #202 remains `hardware_validation`.
