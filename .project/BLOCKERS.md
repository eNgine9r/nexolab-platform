...
**Active blocker: TG-04 / #825 cannot proceed to a real TestLAB send until Issue #869 is merged GREEN and its production no-send dry-run passes.** Real signed iOS Mini App acceptance is PASS on exact snapshot `76957482-57c4-4daf-ac30-d8592847cfbd`, but the existing long-running Telegram worker remains unacceptable for the next gate because it can discover and drain multiple eligible snapshots when delivery is enabled. #869 therefore owns the exact-snapshot, at-most-one-call one-shot send guard and production no-send dry-run prerequisite.

The earlier Codex usage-capacity wait is **cleared by explicit Product Owner direction on 2026-09-04** to continue #869 in the current ChatGPT session. This changes only the implementation executor, not the safety contract. Local software evidence is GREEN: full Telegram Gateway suite `73/73` offline with `--network none` and read-only source mount, focused one-call fake-transport acceptance `3/3`, exact canonical payload-hash contract aligned with Telemetry Service, and candidate gateway image build success. Do not bypass #869 by manually enabling the persistent worker, directly calling Telegram, or broadening scope.

Telegram Gateway remains delivery OFF / Mini App ON / worker stopped / `last_send_at=null`; `DAILY_REPORTS_SCHEDULER_ENABLED=false`. No TestLAB message has been sent in the current acceptance cycle. Remaining #869 blockers are repository-side commit/PR, exact-head required CI and GREEN merge, followed by the production no-send dry-run. Only after that proof may Product Owner approval be requested for one controlled TestLAB send.
