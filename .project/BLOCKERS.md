...
Issue #933 is the active independent software Work Package and has no external blocker. PR #976 exact head `05b3833c1fcaa007ffbd6292dae127c31afa8a62` reached 9/9 routed workflows GREEN including NEXOLAB Merge Gate, but post-GREEN review found a valid P2 telemetry sequence gap when retrying the same already-enqueued event after `flush_queue()` SQLite contention. The local fix makes telemetry sequence allocation plus outbox insert one idempotent `BEGIN IMMEDIATE` transaction keyed by `event_id`; retries reuse the existing queued payload/sequence and operational stream sequencing remains unchanged. Regression evidence proves contiguous `[1, 2]` across flush-busy adaptive retry, duplicate event replay, and transient lock recovery. Verification is 57/57 targeted, 246/246 repository-shaped source, and 246/246 freshly built packaged image with `/app` provenance, all offline/disposable. Production deployment remains intentionally not executed. Remaining gate: commit/push this sequence-integrity fix, resolve the P2 review thread with evidence, fresh exact-head routed CI + NEXOLAB Merge Gate, zero blocking review findings, final Team Lead review, then squash-merge PR #976. #930 remains independent lower-risk maintenance. #189/#585 remain blocked; #201 remains `needs_validation`; #202 remains `hardware_validation`.
