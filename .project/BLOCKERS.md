...
**Repository-side security blocker cleared 2026-09-05.** First exact-head no-cache scan `33957510014` correctly failed closed on two stale `libexpat1 / CVE-2026-66046` exceptions, one in Device Agent and one in Telegram Gateway; both were removed. Corrected head `56698ab43c2b8ffa99db5f7a3085cbc41f281e86` passed Container Supply Chain run `33958808993` with 51 Telemetry, 20 Device Agent and 20 Telegram Gateway HIGH exceptions exactly matched and no CRITICAL/stale/unmatched findings, plus Core CI / NEXOLAB Merge Gate run `33958808977` GREEN. The remaining 91 exact exceptions expire `2026-09-12`; CVE-2026-78409 explicitly records the current Red Hat CNA versus Debian Tracker affected-version disagreement and bounded runtime reachability. Final PR #910 state-only exact-head verification/merge is normal release gating, not a technical blocker. No production deployment, runtime mutation, Modbus/hardware write or data/volume change is authorized by #909.
