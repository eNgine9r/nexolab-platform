...
**Repository gate in progress; production mutation remains a later hard gate.** PR #904 hardens the Gateway refresh so approval is bound to one immutable prebuilt image with exact OCI source revision/tree and a behavioral boundary probe before mutation. A provisional pre-merge proof on `d6d3332bc43764831cc27b3a8c43e731965df87e` produced `sha256:83aa8f8e82cf812d6bb0c8f6970b0cb6818cdf0574d8cb1b01689e3249d22477` and PASSed the boundary probe while deployed `sha256:16a78adc648ddc84a74d3098e8bb8338d3cd5349c6a055b801dc93ee675447c6` FAILed as expected; this remains non-authoritative post-merge evidence only. Successive review findings are now remediated locally: immutable Git-object build context plus source-tree label, fresh `origin/main` authority, standalone pre-approval boundary proof, exact-`SOURCE_SHA` helper execution via `git show`, and the latest P2 that the sudo-run production guard must fetch Git authority as the invoking repository user rather than root. Local code candidate `a0c0dd16ca836605126a1966c41075b1cf574390` requires valid nonroot `SUDO_UID/SUDO_GID`, verifies that account owns the repository, and runs `git fetch origin main` through `runuser` before root-only runtime checks. Focused TG-04 safety is 47/47 PASS; shell syntax and `git diff --check` pass. The last heavy local canonical run remains GREEN on `dda39db7b9ddd1486163975a8a483eb50e2c1eb6` (143/143 frontend files, 697/697 tests, production build, central/edge Compose), but that result is not claimed for `a0c0dd16...`. A repeated heavy detached verifier is intentionally avoided on the production 4 GB Pi while its 2 GB swap is saturated; fresh exact-head GitHub CI/Codex review is therefore mandatory merge authority. After GREEN review/CI and merge, rebuild from exact freshly synced merged `main`, rerun the full read-only outbox/core/Tailscale/rollback preflight, and only then request explicit Product Owner approval for Gateway-only production recreation. Delivery and scheduler remain OFF; no Telegram send or recurring activation is authorized.
