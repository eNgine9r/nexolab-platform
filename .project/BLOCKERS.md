...
**Runtime recovery blocker cleared 2026-08-30.** Rebaseline `20260830T083125Z` established addressable recovery image `sha256:1c639cc7...` from the unchanged healthy running container with sanitized export/import evidence; independent recovery-image tar audit confirms zero mounted payload entries under `/var/lib/nexolab` and `/host/dev`. CI-governance prerequisite #770 is completed and PR #771 merged GREEN after exact-head Core CI, Authenticated Dashboard Acceptance, Offline Bundle and Refrigeration Browser Acceptance all passed. The #768 repository candidate is active again. Its completed-restore authority, superseded-pointer and exact PostgreSQL-volume findings are corrected. Final review then found two additional lifecycle defects: current rebaseline authority did not recheck writable-layer drift, and a successful deployment did not persist the newly activated Device Agent image for the following recovery cycle. Both are now corrected fail-closed: `docker diff` must still match the immutable rebaseline record, and successful evidence records/reuses the exact addressable healthy Device Agent image. The future-cycle boundary is also closed: a valid superseded current pointer may be replaced by a new rebaseline while immutable history is retained; same-source, malformed, or concurrently changed pointers fail closed. The final restore-path P1 is also corrected: guarded SQLite restore launches its helper from the exact addressable recovery image recorded in snapshot metadata, not the unaddressable historical source-container image. The latest exact-head review additionally requires live `device_mode=modbus`; simulator mode is fail-closed, and live Pi health verifies production is currently `modbus`, MQTT-connected with queue depth 0. If successful deployment evidence retains an image ID that later becomes unaddressable, a matching same-source current rebaseline is now resolved and its addressable recovery image is used instead. Local bounded deployment/recovery coverage is 102/102 and the live current-authority resolver passes; the candidate must now receive final exact-head GREEN CI/review and merge before #760 starts. This remains a repository lifecycle gate, not permission to run #760 from the #768 branch.
