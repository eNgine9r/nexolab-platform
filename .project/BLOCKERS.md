...
**Active repository hardening; production mutation remains a hard gate.** The currently deployed persistent Gateway image `sha256:16a78adc…` is topic-aware but behaviorally fails the new recurring-activation boundary by discovering an unapproved pre-cutoff synthetic snapshot. Fresh #901 review also found the existing refresh guard built the target only after approval, so approval was not bound to an immutable target image. Local remediation adds an exact prebuilt target-image pin, exact OCI source-revision proof and the behavioral boundary probe before mutation, and removes post-approval image build. Focused TG-04 deployment safety is 45/45 PASS. After this repository candidate is merged GREEN, #901 must run a fresh exact-main build/read-only production preflight, fingerprint the existing two-row sent outbox, preserve core container/Tailscale identities, and prepare rollback evidence while delivery and scheduler stay OFF. Only then may explicit Product Owner approval authorize the Gateway-only production recreate. #901 does not authorize a Telegram send or recurring activation.
