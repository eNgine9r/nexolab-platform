...
Issue #933 remains active. Exact head `bf4068ce62a3a1f92c58a37acfdae5729db6f170` passed all non-dashboard routed lanes; Authenticated Dashboard attempt 1 failed one unrelated Live Data WebSocket retry timing assertion while same-SHA attempt 2 passed. Post-push review thread `PRRT_kwDOTe72wM6ghnaX` identified a valid P2 recovery-accounting defect: after a write operation exhausted SQLite retries, an unrelated successful read could clear `consecutive_exhaustions` and increment `busy_recoveries_total` even though the write path was still blocked. The local fix uses a per-operation unresolved-exhaustion ledger in both `OfflineQueue` and `LatestValueStore`; only the matching operation can clear its prior exhaustion. Deterministic `BEGIN IMMEDIATE` regressions prove unrelated read success does not mask unresolved write contention, and matching write recovery is counted exactly once. Verification is GREEN at 19/19 SQLite recovery-module tests, 254/254 full source and 254/254 fresh packaged-image tests with `/app` provenance, all offline. Remaining blocker is repository verification only: commit/push, resolve the P2 thread, fresh exact-head routed CI including Container Supply Chain, Offline Bundle and NEXOLAB Merge Gate, zero blocking review findings, final Team Lead review, then merge. Production deployment remains intentionally not executed. #930 remains independent lower-risk maintenance. #189/#585 remain blocked; #201 remains `needs_validation`; #202 remains `hardware_validation`.
