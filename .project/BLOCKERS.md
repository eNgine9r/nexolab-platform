...
Issue #988 is the active critical interrupt after repeated loss of Raspberry Pi Connect and Remote Desktop Commander. Host implementation and controlled reboot acceptance are GREEN: Desktop Commander now runs as its own enabled systemd user service/cgroup with automatic restart and linger; restarting Raspberry Pi Connect no longer terminates it; killing Desktop Commander is automatically recovered; and a full authorized reboot restored Desktop Commander, Raspberry Pi Connect, Tailscale, NetworkManager, all 12 healthy NEXOLAB containers, Dashboard/Telemetry readiness and advancing Device Agent acquisition. Persistent journald now retains previous-boot evidence. The remaining gate is repository PR/exact-head CI/merge, not additional host mutation.
