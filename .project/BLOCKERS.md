...
**Hard blocker: real signed Telegram phone acceptance.** The Product Owner-approved corrected Mini App cutover passed on exact source `9a3556b25b257396d15db80af591d1cc3684b8f7` using verified ARM64 artifact `9918720619`; authoritative deployment evidence is `runtime/deployments/20260904T032305Z`. Source adoption is recorded as `controlled_source_deployment`, schema remains `20260902_0031`, persistent volumes are exact-match 6/6, and the pre-cutover edge SQLite snapshot is consistent with queue `0`.

Post-cutover Dashboard, Telemetry, MQTT and Device Agent are healthy; samples are advancing, workers are 2/2 healthy and no continuing startup retry growth was observed. Telegram Gateway is `ready` with `delivery_enabled=false`, `miniapp_enabled=true`, `running=false`, `last_send_at=null`; `DAILY_REPORTS_SCHEDULER_ENABLED=false`; tailnet `:8444` serves the corrected production Dashboard and `/telegram-miniapp` is HTTP 200. The next required physical action is to open the exact Direct Link for snapshot `76957482-57c4-4daf-ac30-d8592847cfbd` inside the authorized iOS Telegram client and confirm that the persisted report renders instead of `Некоректний формат звіту`. Do not send to TestLAB or enable the schedule until this phone gate passes.
