...
RFX-01 / #717 and RFX-02 / #935 are completed through GREEN PRs #934 and #936. RFX-03 / #937 is the active software Work Package on `feat/937-rfx03-humidity-acquisition`, based on merged main `6b63518f...`. Fresh Codex review of exact head `683aaea...` raised one P2 on source/profile compatibility; it is locally remediated with explicit profile→acquisition-source linkage and a pre-scaling fail-closed guard. Post-remediation non-PostgreSQL evidence is GREEN at `44 passed / 11 PostgreSQL-only skipped`; migration head remains `20260906_0034`, offline PostgreSQL SQL generation is `2842` lines, and a new automated PostgreSQL migration round-trip test is included. Because migration/database code changed, the previous local `10/10` PostgreSQL and round-trip evidence is pre-remediation only; new exact-head PostgreSQL CI, routed workflows, Merge Gate and fresh Codex review are required before merge. #189/#585 remain blocked, #201 remains `needs_validation`, and #202 remains `hardware_validation`.
