...
**Hard blocker: separate Product Owner production/site cutover approval.** Software repair #854 is merged GREEN and the corrected frontend pre-cutover candidate is verified. Exact deployment target is `9a3556b25b257396d15db80af591d1cc3684b8f7`; Frontend Release Artifact run `33821653923` produced artifact `9918720619` (`sha256:dc7fc9182ee9929d0f4c507aa989c03cb1845d4ea5a3eeb1d977902894b8781e`), staged at `runtime/staging/tg04-schema-fix-9a3556b2`. Artifact identity/public-contract/archive checks and guarded source-selection preflight passed. Isolated loopback verification returned HTTP 200 for `/`, `/nodes`, `/live`, `/energy`, `/sessions` and `/telegram-miniapp`, with the active Dashboard unchanged and candidate cleanup complete.

Production is still source `13ab26392fcb1c1385dca3c1f619da4512fe568c`, schema `20260902_0031`. Telegram Gateway remains `ready` with `delivery_enabled=false`, `miniapp_enabled=true`, `running=false`, `last_send_at=null`; `DAILY_REPORTS_SCHEDULER_ENABLED=false`. No corrected frontend/source activation has occurred. After state-only #862 merges, rerun source-selection preflight against resulting `main`, then stop for Product Owner approval. If approved, use only the guarded NEXOLAB deployment path with delivery and scheduler still OFF, then repeat the exact real signed Mini App persisted-report acceptance. One controlled TestLAB send, reliability/no-duplicate evidence and Monday–Friday 07:50 `Europe/Kyiv` activation remain later explicit gates.
