...
**Active repository prerequisite for #883.** Exact #883 candidate `3b637bba4f9042772177e2661f43abe015c483bf` passed its product/module/Core checks through the production build, then the repository-owned local candidate verifier produced a deterministic false RED by running Compose overlays standalone. Safe synthetic interpolation moved the failure from `compose.broker-control-acceptance.yaml` to `compose.browser-acceptance.yaml`, which is invalid without its base bundle by design. #884 changes only CI-governance verifier/tests: canonical central and edge contracts remain baseline checks, a changed Telegram overlay is validated as `compose.central.yaml + compose.telegram.yaml`, and unregistered changed Compose files fail closed. No runtime, Telegram, hardware, Modbus or production mutation belongs to #884.

## Issue #825 — TG-04 real Telegram acceptance

**First real TestLAB one-shot delivery is cleared; #825 is temporarily blocked on #883 topic routing, which is itself blocked only on #884 verifier repair.** Real signed iOS Mini App acceptance is PASS on exact snapshot `76957482-57c4-4daf-ac30-d8592847cfbd`. Issue #869 / PR #872 passed the full exact-head matrix and merged GREEN. The production no-send dry-run returned `dry_run_ready`, exact payload SHA match, `delivery_state=absent`, `duplicate_risk=false`, and zero outbox mutation. After separate Product Owner approval, exactly one real send succeeded: durable outbox state is `sent`, Telegram message ID is present, duplicate risk is false, and the payload digest matches. Product Owner visual evidence confirms the report appeared in TestLAB General.

Telegram Gateway remains delivery OFF / Mini App ON / worker stopped / `last_send_at=null`; `DAILY_REPORTS_SCHEDULER_ENABLED=false`. Do not enable the persistent worker or scheduler yet. After #884 merges, rebase/integrate current `main` into #883, rerun exact-head local/remote verification, merge GREEN, then perform protected topic capture and a no-send dry-run before the separately authorized single topic test. Recurring weekday 07:50 activation remains later.
