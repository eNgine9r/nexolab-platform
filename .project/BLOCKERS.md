...
Issue #933 is active after #978 / PR #979 cleared the cross-image supply-chain interrupt and merged to `main` `7252295de3d76dab1f3554b1f257918f1e9a5891`. Candidate `6764370ce89ac0b81aa839a6cc0941bc90509ac4` passed 57/57 targeted, 246/246 source and 246/246 packaged tests, but PR #976 has an unresolved **P1** review thread `PRRT_kwDOTe72wM6gfnXC`: after `LatestValueStore.record_attempt()` exhausts retryable SQLite contention, the scheduler can advance and publish/queue the same result while the latest-value read model remains stale. Merge is blocked until the exact latest update is retained/retried or the runtime fails closed before loss, regression coverage passes, the thread is resolved, and a fresh exact-head routed CI/NEXOLAB Merge Gate is GREEN. #930 remains independent lower-risk maintenance. #189/#585 remain blocked; #201 remains `needs_validation`; #202 remains `hardware_validation`.

## Issue #978 — new libexpat1 HIGH supply-chain gate

**Cleared security-policy blocker; completion candidate 2026-09-09.** Trigger run `34307375095` correctly failed closed on exactly four new HIGH tuples: Device Agent and Telegram Gateway each contained `libexpat1 2.8.3-1~deb13u1` with `CVE-2026-76956` and `CVE-2026-76957`. Debian Trixie/security remains vulnerable/no-DSA while fixed Expat 2.8.4 is not yet consumable from stable Trixie; source/runtime review found no XML parser path in either service. Four exact exceptions expiring `2026-09-12` were added. PR #979 substantive head `e90b034a04d47ce5c2eae3c2833d06b573cf7fe8` then passed fresh no-cache Container Supply Chain `34309529777`, Core/NEXOLAB Merge Gate `34309529802`, Telemetry Service `34309529811`, and has zero review threads. Only exact-current-head revalidation of the final state reconciliation remains before merge. No production deployment, runtime mutation, Modbus/hardware write, data deletion or volume deletion is authorized or required.
