...
## Sprint execution gate — Issue #988 reliability interrupt; Issue #965 paused
...
Issue #988 is the active critical interrupt after repeated remote-access loss. Real host reboot/service recovery is GREEN, and the later Desktop Commander `0.2.48` stale rotated-refresh-token failure is now mitigated by an exact temporary upstream PR #685 source pin `7edee255c17101bfe50f684bd61e7f818e552205`, immutable release layout, provenance/effective-policy verification and transient self-update handoff. ARM64 synthetic rotation tests are 17/17 PASS and the live self-update returned `status=success` with `NRestarts=0`; staged release verification now runs before immutable promotion, with reliability tests 11/11 PASS. Remaining gate: push the exact `a09360948ae75d4deaf088d7ffc9e090fc150e05` implementation plus state update, obtain new exact-head CI/review with zero unresolved threads, then merge. No further host mutation is required for the current acceptance.
Issue #965 is temporarily `blocked` only to preserve the one-primary-work-package rule while #988 completes. Its existing feature checkout and verification evidence are preserved; it is the next Work Package immediately after #988 merges. This is not a product dependency failure. K96–K100 remain hardware-unverified and excluded from automatic production polling. #189/#585 remain blocked, #201 remains `needs_validation`, and #202 remains `hardware_validation`.
