...
The #759 implementation candidate now captures a live database with Python `sqlite3.Connection.backup()`, verifies source and snapshot with `PRAGMA quick_check`, records sanitized hash/size/revision/queue/high-water/stream-sequence evidence, and provides an explicit atomic restore that rejects a running Device Agent, wrong evidence, corruption, SQLite sidecars and queue/sequence advancement that could contain newer state. The helper is checksum-staged before historical checkout and capture runs immediately before the runtime-mutation marker. Focused snapshot/deployment regressions pass 77/77 and the standalone offline runtime contract passes. A read-only proof using implementation SHA `c0508b9b67253972bced99d6df11f63ccc030a74` against the live `ff86b10b...` edge database is retained outside Git at `runtime/evidence/issue-759-edge-snapshot-20260829T191752Z`: source/snapshot quick checks are `ok`, size is 253,952 bytes, registry revision is 18, queue depth is 0, queue high-water is 6,505,906 and snapshot SHA-256 is `a0e6ebef6b2d64516a6b151098d4c439610d564a43724fdfd95a88c8661776cb`. No container restart, runtime mutation, Modbus action or hardware write occurred.
