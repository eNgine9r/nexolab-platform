...
`PRODUCTION-READINESS-1` remains active only for outstanding hardware/recovery acceptance boundaries. Its repository gate is complete. RFX-00 / Issue #715 is in final review in PR #716. ADR 0010 is accepted as the architecture authority for the next RFX implementation Work Packages, but #715 is not complete until the accepted-ADR exact head passes required CI/NEXOLAB Merge Gate and merges.
