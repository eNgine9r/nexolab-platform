...
Security maintenance is being reconciled under Issue #704 / PR #705. Fresh exact-head scan `33015137554` confirms telemetry-service consumes fixed OpenSSL `3.5.7-1~deb13u2`, so its three `CVE-2026-14456` exceptions remain retired; Device Agent retains only `libssl3t64/CVE-2026-14456` through **2026-08-30** while the supported distroless base still lags the fixed Debian package. Exact Device Agent `libsqlite3-0` HIGH decisions (`CVE-2026-11822`, `CVE-2026-11824`) remain reviewed through **2026-09-02**. The same fresh PR #705 telemetry image exposed four additional HIGH tuples now covered by exact short-lived candidate decisions through **2026-09-02**: telemetry `libsqlite3-0/CVE-2026-11822`, `libsqlite3-0/CVE-2026-11824`, `libcjson1/CVE-2026-16554`, and `libwebsockets19t64/CVE-2026-78161`. Repository/upstream review finds the SQLite FTS5 + arbitrary-SQL/crafted-DB prerequisite absent; cJSON requires a 32-bit path while controlled NEXOLAB images are amd64/arm64; and libwebsockets 78161 is described for 4.5.0 LECP CBOR Recording while the image contains 4.3.5 and NEXOLAB does not invoke that component. These are pending exact-head acceptance decisions, not completed release evidence: a new fresh no-cache Container Supply Chain run must still pass after the corrected candidate is pushed. Remove any decision earlier when its finding disappears, a supported fix becomes consumable, reachability/version evidence changes or severity becomes Critical.
