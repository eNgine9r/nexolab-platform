...
Issue #683 and #686 recovery hardening are completed, including replacement post-merge ARM64/local-auth dispatch `32939760743` for runtime source `cc27b609...`. Issue #707 is merged and its controlled LAN deployment now runs source `ff796b1f7ddcf95a9be8e7f93c75a4837ec7eb0d`. The Product Owner confirmed the consolidated graphs and CSV download path work, but the downloaded CSV lacked sensor readings. Read-only production inspection traced this to catalog `DIXELL-*` identity being used against persisted temperature telemetry keyed as `K*`; focused regression Issue #709 is active. The previously accepted recovery artifact `9584581740` remains historical evidence for `cc27b609...` but is not exact-source recovery authority for the currently deployed `ff796b1f...` lineage. Critical performance Issue #696 and hosted-runner recovery Issue #698 are completed; Issue #690 remains Ready and resumes after #709.
...
Issue #709 is the active critical regression interruption. Production evidence on deployed `ff796b1f...` shows Saved Dashboard `108-01 / temperature.probe` resolves through catalog device `DIXELL-108`, while the persisted 24-hour telemetry series is keyed as `equipment_id=K108` with roughly 1440 samples. The export query therefore returned a valid downloadable CSV without sensor rows. The focused branch `fix/709-live-dashboard-telemetry-equipment-id` introduces one deterministic Live Dashboard telemetry identity mapping (`temperature_controller -> K{unit_id}`, `energy_meter -> LE01MP-{unit_id}`), applies it to export and inventory/latest lookup, and fails closed for unknown device types. Exact candidate `93cf964712eb8df426e573646a31d3cd81937769` has `py_compile`, `git diff --check`, a targeted 7-test Raspberry Pi isolated-container regression run, and the full Live Dashboard module suite at 23 passed / 1 skipped. PR #710 is open; exact-head remote CI/review is still pending and the fix is not yet deployed.
