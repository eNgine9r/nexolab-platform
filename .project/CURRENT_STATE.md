...
`PRODUCTION-READINESS-1` remains active for outstanding hardware/recovery acceptance boundaries and the current RFX-00 architecture review. Its production-readiness repository gate is complete. RFX-00 / Issue #715 remains in final review in PR #716 after two additional architecture findings: ADR 0010 now persists `observation_at` for reproducible freshness decisions and enforces non-overlapping semantic-role binding intervals so each required calculation role resolves to exactly one accepted signal. Previous exact-head evidence remains intentionally invalidated until this corrected accepted ADR passes required CI/NEXOLAB Merge Gate and fresh review.
