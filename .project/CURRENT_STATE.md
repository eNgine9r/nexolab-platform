...
Issue #665 is the active state-continuity Work Package. It closes the exhausted `CHART-RELIABILITY-1` queue after #415/#663 completion and records the remaining validation boundaries without changing runtime behavior.

Next critical validation target: Issue #444 — **Restore LOCAL_LAN user administration API availability**.
...
- #646 — repository-side Merge Gate exists; GitHub main branch protection/rules remain a settings-access blocker.

## Security maintenance

Four exact OpenSSL QUIC `CVE-2026-14456` temporary decisions remain reviewed through **2026-08-30**. Debian Trixie still exposes OpenSSL `3.5.6-1~deb13u2` as vulnerable/postponed; upstream fixes the affected 3.5 line in 3.5.8. The lifecycle document contains an older 2026-08-26 sentence above the later Stage 2 renewal; do not treat that stale sentence as the current decision. Reconcile it in the next focused security review WP rather than broadening #665.
...
No Modbus/controller write, hardware write, product persistent-data deletion, named-volume deletion, secret exposure, DNS/billing mutation, production deployment, network isolation or reboot is authorized by Issue #665.
