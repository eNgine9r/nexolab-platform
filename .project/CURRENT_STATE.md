...
The #759 implementation candidate now captures a live database with Python `sqlite3.Connection.backup()`, verifies source and snapshot with `PRAGMA quick_check`, records sanitized hash/size/revision/queue evidence, and provides an explicit atomic restore that rejects a running Device Agent, wrong evidence, corruption and SQLite sidecars that could contain newer state. Focused snapshot/deployment regressions pass 74/74 and the standalone offline runtime contract passes. A read-only proof using implementation SHA `cb4f2110fd720a7bb12fa397ac1f5fb700b9ef71` against the live `ff86b10b...` edge database is retained outside Git at `runtime/evidence/issue-759-edge-snapshot-20260829T184419Z`: source/snapshot quick checks are `ok`, size is 253,952 bytes, registry revision is 18, queue depth is 0 and snapshot SHA-256 is `5300ce4e0dbdf820c40cf52e81c7e9e50cba242943c31b8d779ace29e558f0ac`. No container restart, runtime mutation, Modbus action or hardware write occurred.
