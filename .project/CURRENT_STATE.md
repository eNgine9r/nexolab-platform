...
The Product Owner authorized the next Embraco/controller production stage after #755 completed. Fresh audit keeps completed Issue #729 as the accepted software implementation and splits production activation into two focused Work Packages. Issue #759 is active and must make the edge SQLite acquisition-registry transition rollback-safe before any Embraco enrollment or Bus 2 runtime mutation. Issue #760 is blocked on #759 and owns the later bounded production activation/acceptance of Embraco Unit 2.
...
The #759 implementation candidate now captures a live database with Python `sqlite3.Connection.backup()`, verifies source and snapshot with `PRAGMA quick_check`, records sanitized hash/size/revision/queue/high-water/stream-sequence and exact pre-cutover Device Agent image evidence, and provides an explicit atomic restore that rejects a running Device Agent, wrong evidence, corruption, an unavailable previous image, SQLite sidecars and queue/sequence advancement that could contain newer state. Restore retags and verifies the previous Device Agent image before database replacement; restart remains separate and force-recreates onto that image. The helper is checksum-staged before historical checkout and capture runs immediately before the runtime-mutation marker. Focused snapshot/deployment regressions pass 78/78 and the standalone offline runtime contract passes. A read-only proof using implementation SHA `2c3e6288e7cc03dd230d130bdd914f5adf43fa8e` against the live `ff86b10b...` edge database is retained outside Git at `runtime/evidence/issue-759-edge-snapshot-20260829T192812Z`: source/snapshot quick checks are `ok`, size is 253,952 bytes, registry revision is 18, queue depth is 0, queue high-water is 6,506,603, the exact Device Agent image ID is recorded, and snapshot SHA-256 is `584b038a0c47450b4b2e572891d699bf31623f9117fa303c94c50ed20824c73e`. No container restart, runtime mutation, Modbus action or hardware write occurred.
