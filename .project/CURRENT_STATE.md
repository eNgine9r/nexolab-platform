...
## Issue #730 Opera/Tailscale inspection access

Issue #730 / PR #731 remains in review after a review-driven security hardening pass. The original actual-host acceptance proved the dedicated read-only viewer workflow, same-origin private Tailscale HTTPS path and service-level restart, but review correctly identified that the first loopback `/inspection-login` implementation trusted proxy headers on a normal TCP listener. That credential-exchange route has been removed from port `3100`. The accepted candidate now routes only `/inspection-login` through a root-owned Unix socket (`0600 root:root`); the socket-activated helper additionally requires root `SO_PEERCRED` plus exact Tailscale user/source/HTTPS/host evidence before reading the host-local credential or calling canonical local auth. The helper, systemd templates, placeholder config and focused policy tests are tracked in the repository without populated identity values or secrets.

Actual-host negative evidence confirms the privileged socket is enabled/active, the normal inspection frontend remains enabled/active with `Linger=yes`, an unprivileged local socket connection is denied, direct `127.0.0.1:3100/inspection-login` returns HTTP 404, and a real tailnet request from the Raspberry Pi under a non-approved Tailscale identity returns HTTP 403 through the Unix-socket helper. The existing `/` and `/api` Serve handlers remain unchanged and `/inspection-login` points to the Unix socket. A full host reboot is not claimed; deterministic service/socket recovery is documented instead. The final approved-workstation Opera/Tailscale positive recheck is pending because Opera Browser Connector was disconnected at the last verification attempt. This is a soft tooling blocker only; no core NEXOLAB runtime dependency is introduced.
