...
`PRODUCTION-READINESS-1` remains active for outstanding hardware/recovery acceptance boundaries and the current RFX-00 architecture review. Its production-readiness repository gate is complete. RFX-00 / Issue #715 remains in final review in PR #716. ADR 0010 now additionally defines half-open `[valid_from, valid_to)` semantic-binding intervals, deterministic as-of source-sample selection/tie-breaking, and immutable historical provenance for instrument, acquisition/scaling-profile and calibration quality gates. Previous exact-head evidence remains intentionally invalidated until this corrected accepted ADR passes required CI/NEXOLAB Merge Gate and fresh review.
