...
Issue #772 / PR #773 is completed and merged, clearing the persisted-bus safety prerequisite. The Product Owner clarified that production acceptance uses **physical controller #2 only**, mapped to Embraco Unit ID 2 on `rs485-embraco`; controller #1 is test-only and Unit 96 is excluded.

Repository-owned controlled deployment `runtime/deployments/20260830T202942Z` passed on exact source `20bb9ca473395a0c64267b9b08523c31404f41e6` using verified ARM64 frontend artifact run `33330031287` / artifact `9737449091`. Pre-cutover SQLite snapshot SHA-256 `a0d2fdd29d9d51c356476215b8d25aa9a8c8a79915716cdfd684ba5c5c9e56ab` captured revision 18 with quick-check OK before mutation. Post-cutover schema head is `20260828_0027`; persisted registry is revision 20 with `embraco-2` active on `rs485-embraco` and LE-01MP Unit 201 still disabled on `rs485-main`.

Real Unit 2 FC03-only telemetry is live and durable. Device Agent remains healthy in `modbus` mode, MQTT is connected and queue depth is zero. PostgreSQL latest/history contains verified compressor speed, control state, relay bits and alarm bits. Temperature, setpoint and hysteresis values remain NULL/`unknown`, so unverified engineering scaling is not presented as production data. `Cool jet` is bound to `EMBRACO-2` through the domain repository with security audit event `abe4e8a9-bd8b-4e35-b182-06fd6c8330a3`. Long-run evidence on 2026-08-31 observed 9,191 Embraco rows; the latest one-hour compressor-speed window had 60 valid samples, maximum gap about 60.0 s, approximately 98.3% observed coverage and approximately 59.3% duty.

The immutable production release is also restored on the tailnet inspection mirror at `127.0.0.1:3100` with HTTP 200. A real headless UI pass reached `Cool jet`, controller summary, all detail tabs, live RPM and `Scale unverified`; its duty assertion raced the asynchronous history load rather than proving a product failure. The final authenticated browser rerun remains externally blocked because Opera Browser Connector is disconnected and the alternate ephemeral credential-helper route is stopped by the tool safety guard. Repository-owned source/version authority adoption is likewise pending because the privileged adoption command is blocked before execution by the tool safety guard. These are the only remaining #760 acceptance gates; no credential or privileged-command guard may be bypassed.
