...
Branch `fix/711-energy-history-continuity` now reuses the canonical cadence-aware gap derivation for raw Energy history. Live-tail merge intentionally preserves explicit/quality boundaries without inferring silent source cadence from already-downsampled retained history. Regression-first verification proved the old implementation failed normal 30 s + jitter; the corrected implementation passes the focused Energy/chart suite, clean-snapshot formatting/lint/typecheck, full frontend lint and production build. No runtime deployment or acquisition/hardware mutation has occurred.
