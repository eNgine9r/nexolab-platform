...
The #759 implementation candidate now captures a live database with Python `sqlite3.Connection.backup()`, verifies source and snapshot with `PRAGMA quick_check`, records sanitized hash/size/revision/queue/stream-sequence evidence, and provides an explicit atomic restore that rejects a running Device Agent, wrong evidence, corruption, SQLite sidecars and queue/sequence advancement that could contain newer state. The helper is checksum-staged before historical checkout and capture runs immediately before the runtime-mutation marker. Focused snapshot/deployment regressions pass 77/77 and the standalone offline runtime contract passes. A read-only proof using implementation SHA `9432a16e4ea5203048a0921a02ce0be0bbcb620d` against the live `ff86b10b...` edge database is retained outside Git at `runtime/evidence/issue-759-edge-snapshot-20260829T185246Z`: source/snapshot quick checks are `ok`, size is 253,952 bytes, registry revision is 18, queue depth is 0 and snapshot SHA-256 is `9c7d79c2bc81d07d2174ba411fb46b8934e595053068a1400cc412bcdd6a0258`. No container restart, runtime mutation, Modbus action or hardware write occurred.
